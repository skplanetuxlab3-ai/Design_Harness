...
### ⚠ Figma 경로 주석 ≠ Figma 변수 (2026-09-01 확인)

`spacing.css` 의 `/* products/deal/items/padding_top */` 같은 주석 225개는
**변수 참조가 아니라 "레이어 어디서 이 숫자를 읽었는지" 적어둔 메모다.**

**변수 조회는 노드 스코프다. 몇 개 떠보고 "PDS 에 없다" 고 결론내지 마라.**
딜 카드·배지·태그 등 쇼핑 컴포넌트 6개를 떴을 때는 spacing 변수가 3개뿐이었다.
그런데 적립쇼핑 화면(`19550:129755`) 하나에서 **124개**가 나왔다 — spacing 11단계,
radius 7단계, elevation 3종, filled/* 버튼 토큰까지. 화면 단위 노드가 컴포넌트
노드보다 훨씬 많은 변수를 물고 있다.

딜 카드에 패딩 변수가 없던 건 PDS 에 없어서가 아니라 **그 컴포넌트가 안 쓰고
있어서**였다 (오토레이아웃 값으로 직접 박혀 있었다).

따라서:

- **`/* figma/path */` 주석이 있다고 드리프트 검사가 그 값을 감시하는 게 아니다.**
  감시 대상은 `scripts/figma-vars/*.json` 스냅샷에 실제로 잡힌 변수뿐이다.
- 새 토큰에 Figma 경로 주석을 달 때, 그 경로가 진짜 변수인지 확인하지 않았다면
  달지 말 것. 대신 `코드 결정 · Figma 변수 없음` 이라고 적는다.
- Figma 변수 승격은 값을 옮기는 작업이 아니라 **감사(audit)** 다. 딜 카드 5개를
  올리는 과정에서 1px 좌표 드리프트, 토큰 이름 중복, 네임스페이스 불일치가 나왔다.
  비용이 크므로 컴포넌트 단위로 골라서 한다. 남은 목록은
  `node scripts/token-decisions.mjs --pending`.
...
⚠️ 페이지 단위 노드는 메타데이터가 너무 커서 `get_metadata` 가 실패한다 —
`쇼핑`(`1401:42907`), `적립쇼핑`(`19449:147573`, 11045×6063 · 화면 15개 + 컴포넌트 스펙).
`get_variable_defs` 도 같은 이유로 "nothing selected" 를 돌려준다.
화면 하나를 지정하거나, 컴포넌트 페이지 덤프를 grep 하는 쪽이 빠르다.
`get_screenshot` 은 페이지 크기와 무관하게 되므로 무엇이 든 노드인지 볼 때 쓴다.
