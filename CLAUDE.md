...
OCB PDS 에 실제로 존재하는 spacing 변수는 셋뿐이었다 — `products/spacing02`(2),
`products/spacing04`(4), `products/spacing08`(8). radius 4개. 나머지는 전부 색과
타이포다. 딜 카드 노드를 직접 떠봐도 패딩 변수가 나오지 않았다. Figma 에서
오토레이아웃 값으로 직접 박혀 있다는 뜻이다.
